#solved usinng chatgpt

MAXSIZE = 101

# Dynamic programming
matrix = [[0] * MAXSIZE for _ in range(MAXSIZE)]
memory = [[-1] * MAXSIZE for _ in range(MAXSIZE)]
...
def calculate(row, column):
    ...
    memory[row][column] = total + min(calculate(row - 1, column), calculate(row - 1, column + 1))
    return memory[row][column]
